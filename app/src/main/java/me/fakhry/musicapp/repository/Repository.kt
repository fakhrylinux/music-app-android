package me.fakhry.musicapp.repository
...
object Repository {
    ...
    fun getDataTopAlbumsFromAssets(context: Context?): List<Album>? {
        ...
        return try {
            ...
            json = inputStream?.bufferedReader().use { it?.readText() }
            Log.d("Repository", "getDataTopAlbumsFromAssets: $json")
            val groupListType = object : TypeToken<List<Album?>>() {}.type

            Gson().fromJson(json, groupListType)
        } catch (e: IOException) {
            e.printStackTrace()
            Log.e("Repository", "error_getDataTopAlbumsFromAssets: ${e.message}")
    ...
    fun addDataToTopChart() {
        ...
        FirebaseStorage
            .getInstance("gs://music-app-81620.appspot.com")
            .reference
            .child("musics")
            .listAll()
                ...
                listResult.items.forEach { item ->
                    item.downloadUrl
                        ...
                        .addOnFailureListener { e ->
                            Log.e(
                                "Repository",
                                "[addDataToTopChartsImage-downloadUrl] ${e.message}"
                            )
                        }
                }
            }
                ...
                listResult.items.forEach { item ->
                    item.downloadUrl
                        ...
                        .addOnFailureListener { e ->
                            Log.e(
                                "Repository",
                                "[addDataToTopAlbumsImage-downloadUrl] ${e.message}"
                            )
                        }
                }
            }
